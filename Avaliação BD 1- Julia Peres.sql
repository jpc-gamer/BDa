...
values
("Red Bull Racing", "RB20", 798, "1.000 cv", "Honda RBPT", "fibra de carbono", "independente", "OZ Racing"),
...
-- Seleciona as equipes que utilizam fibra de carbono 
...
-- Equipes qu utilizam a suspensão "ar"
select marca from carros where suspensao = 'ar';

-- Equipes qu utilizam a suspensão "independente"
select marca from carros where suspensao = 'independente';

-- Equipes qu utilizam a suspensão "hidropneumatico"
select marca from carros where suspensao = 'hidropneumatico';

-- Contagem de linhas da tabela carros 
select COUNT(*) as total_carros from carros;

-- Listagem de pilotos ingleses
SELECT nome, nacionalidade from piloto where nacionalidade = 'Inglaterra';

-- Listagem de pilotos brasileiros
SELECT nome, nacionalidade from piloto where nacionalidade = 'Brasil';

-- Listagem de pilotos mexicanos
SELECT nome, nacionalidade from piloto where nacionalidade = 'México';

-- Listagem de pilotos japoneses
SELECT nome, nacionalidade from piloto where nacionalidade = 'Japão';

-- Listagem de pilotos americanos
SELECT nome, nacionalidade from piloto where nacionalidade = 'Estados Unidos';

-- Listagem de pilotos que são ingleses e da equipe mercedes 
select nome, nacionalidade, equipe from piloto where nacionalidade = 'Inglaterra' and equipe = 'Mercedes';

-- Nacionalidade dos pilotos da ferrari 
select distinct nacionalidade, nome, equipe from piloto where equipe = 'Ferrari';

-- Exibe quem é o companheiro do piloto Lando Norris
select nome, companheiro from piloto where nome = 'Lando Norris';

-- Exibe os pilotos que possuem mais de 90 pontos 
select nome, pontos from piloto where pontos > 90;

-- Exibe os pilotos que possuem menos de 90 pontos 
select nome, pontos from piloto where pontos < 90;

-- Exibe o piloto na ordem de maior pontuação
select nome, pontos from piloto order by pontos DESC limit 1; 

-- Exibe o piloto na ordem de menor pontuação
select nome, pontos from piloto order by pontos ASC limit 1;

-- Listagem dos pilotos da equipe williams
select nome, equipe from piloto where equipe = 'Williams';

-- Listagem dos pilotos da equipe ferrari
select nome, equipe from piloto where equipe = 'Ferrari';

-- Listagem dos pilotos da equipe MClaren
